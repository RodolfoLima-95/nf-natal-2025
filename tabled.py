...
def process_table(tsv_file):
    name = re.sub(r'^(.*?)\.[^.]+$', r'\1', tsv_file)
    df = pd.read_csv(tsv_file, sep="\t", header=None)
    df.columns = ["ID", "Cromossomo", "Start", "End", "Tipo", "SVLEN"]
    df["Grau"] = df["SVLEN"].apply(lambda x: grade(x))
    return df.to_csv(f'variantes.{name}.csv', sep=',', index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Converte TSV para CSV e adiciona classificação baseada no tamanho da variante."
    )
